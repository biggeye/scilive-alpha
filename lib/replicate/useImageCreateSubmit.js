...
export const useImageCreateSubmit = (supabase) => {
  ...
  const [, setModelBootProgress] = useRecoilState(modelBootProgressState);
  const [, setPredictionError] = useRecoilState(predictionErrorState);
  const [, setPredictionResult] = useRecoilState(predictionResultState);
  const [, setPredictionProgress] = useRecoilState(predictionProgressState);
  const [, setFinalPrediction] = useRecoilState(finalPredictionState);
  const [, setPredictionStatus] = useRecoilState(predictionStatusState);
