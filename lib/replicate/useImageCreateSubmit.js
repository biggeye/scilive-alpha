...
export const useImageCreateSubmit = (supabase, userId) => {
  ...
  const submitImageCreate = async (userInput, modelId, userId) => {
    ...
    try {
      ...
      if (response.status !== 201) {
        console.error("Error in response from /api/replicate", predictionResponse);
        throw new Error(predictionResponse.detail);
      }

      console.log("Response from /api/replicate", predictionResponse);
  ...
  const pollPredictionStatus = async (predictionResponse) => {
    ...
    return { isLoading, submitImageCreate }
