...
import { exampleImageState, selectedModelIdState } from "@/state/replicate/config-atoms";
...
export const useImageCreateSubmit = () => {
  ...
  const [finalPredictionPrompt, setFinalPredictionPrompt] = useRecoilState(
    finalPredictionPromptState
  );
  ...
  const [existingUpload, setExistingUpload] =
    useRecoilState(existingUploadState);
  ...
  const uploadPredictionResult = async (
    ...
    return await uploadPrediction(file, userId, modelId, predictionId, prompt, contentType);
  };
