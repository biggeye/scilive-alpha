...
export const useImageCreateSubmit = () => {
  ...
  const [finalNarrative, setFinalNarrative] =
    useRecoilState(finalNarrativeState);
  ...
  const submitImageCreate = async (userInput) => {
    ...
    const [globalLoading, setGlobalLoading] = useRecoilState<Boolean(globalLoadingState)>(null);
